...
export function PriorityCases() {
  ...
  const { t } = useI18n();
  const { priorityCases, casesLoading } = useDashboardStore();
  ...
  const filteredCases =
    filterStatus === 'all'
      ? priorityCases
      : priorityCases.filter((c) => {
          if (filterStatus === 'urgent') return c.priority === 'urgent' || c.priority === 'high';
          if (filterStatus === 'medium') return c.priority === 'medium';
          if (filterStatus === 'completed') return c.status === 'closed' || c.status === 'won';
          return true;
        });
      ...
      <div className="space-y-4">
        ...
        {!casesLoading && filteredCases.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            <p>{t('DASHBOARD.PRIORITY_CASES.NO_CASES')}</p>
          </div>
        )}
        {!casesLoading &&
          filteredCases.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(ROUTES.CASES.DETAIL(item.id))}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {(item.status === 'closed' || item.status === 'won') && (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(ROUTES.CASES.DETAIL(item.id));
                    }}

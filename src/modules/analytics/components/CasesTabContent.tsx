      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
        {casesByStatus.map((item) => (
          <Card key={item.status}>
            ...
            <div className="text-xs sm:text-sm text-muted-foreground">{item.status}</div>
          </Card>
        ))}
      </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
                      <span className="text-green-600 dark:text-green-400">{item.won} выиграно</span>
                      <span className="hidden sm:inline">•</span>
                      <span className="text-blue-600 dark:text-blue-400">{item.inProgress} в процессе</span>
                      <span className="hidden sm:inline">•</span>
                      <span className="text-red-600 dark:text-red-400">{item.lost} проиграно</span>
                    </div>
                  </div>

...
const casesByStatus = [
  ...
  { status: 'IN_PROGRESS', count: 15, percentage: 24, color: 'text-blue-600', bgColor: 'bg-blue-50', icon: Clock },
  { status: 'LOST', count: 5, percentage: 8, color: 'text-red-600', bgColor: 'bg-red-50', icon: XCircle },
];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
        {casesByStatus.map((item) => (
          <Card key={item.status}>
            ...
            <div className="text-xs sm:text-sm text-muted-foreground">{t(`ANALYTICS.CASE_STATUS.${item.status}`)}</div>
          </Card>
        ))}
      </div>

      <Card>
        <h3 className="text-base sm:text-lg md:text-xl tracking-tight mb-3 sm:mb-4 md:mb-6">{t('ANALYTICS.CASES_BY_PRACTICE')}</h3>
              ...
              <div key={item.practice}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <div className="min-w-0">
                    <h4 className="tracking-tight mb-0.5 sm:mb-2 text-sm sm:text-base truncate">{t(`ANALYTICS.PRACTICE_TYPES.${item.practice}`)}</h4>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
                      <span className="text-green-600 dark:text-green-400">{item.won} {t('ANALYTICS.PRACTICE_STATS.WON')}</span>
                      <span className="hidden sm:inline">•</span>
                      <span className="text-blue-600 dark:text-blue-400">{item.inProgress} {t('ANALYTICS.PRACTICE_STATS.IN_PROGRESS')}</span>
                      <span className="hidden sm:inline">•</span>
                      <span className="text-red-600 dark:text-red-400">{item.lost} {t('ANALYTICS.PRACTICE_STATS.LOST')}</span>
                    </div>
                  </div>

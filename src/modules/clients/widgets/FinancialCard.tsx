...
export function FinancialCard({ financialData }: FinancialCardProps) {
  ...
  return (
    <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
      <CardHeader>
        <CardTitle className="text-white text-base sm:text-lg">{t('CLIENTS.FINANCIAL.TITLE')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3 sm:space-y-4">
        <div className="flex items-center justify-between">
          <span className="font-bold text-blue-100 text-xs sm:text-sm">{t('CLIENTS.FINANCIAL.TOTAL_AMOUNT')}</span>
          <span className="text-lg sm:text-xl font-bold">{totalAmount.toLocaleString('ru-RU')} ₽</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="font-bold text-blue-100 text-xs sm:text-sm">{t('CLIENTS.FINANCIAL.PAID_AMOUNT')}</span>
          <span className="text-lg sm:text-xl font-bold">{paidAmount.toLocaleString('ru-RU')} ₽</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="font-bold text-blue-100 text-xs sm:text-sm">{t('CLIENTS.FINANCIAL.REMAINING_AMOUNT')}</span>
          <span className="text-lg sm:text-xl font-bold">{remainingAmount.toLocaleString('ru-RU')} ₽</span>
        </div>
        <div>
          <div className="mb-2 flex items-center justify-between text-xs sm:text-sm">
            <span className="font-bold text-blue-100">{t('CLIENTS.FINANCIAL.PAYMENT_PERCENTAGE')}</span>

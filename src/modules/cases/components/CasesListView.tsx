...
export function CasesListView() {
  ...
  const stats = [
    { label: 'Всего дел', value: cases.length, color: 'text-blue-500' },
    { label: 'В работе', value: cases.filter(c => c.status === CaseStatusEnum.IN_PROGRESS).length, color: 'text-purple-500' },
    { label: 'Завершено', value: cases.filter(c => c.status === CaseStatusEnum.COMPLETED).length, color: 'text-green-500' },
    { label: 'Срочные', value: cases.filter(c => c.priority === CasePriorityEnum.HIGH).length, color: 'text-red-500' },
  ];
            ...
            <Card key={index} className="bg-white border-0 shadow-sm">
              <div className="p-5">
                <div className="flex items-center justify-between">
                  ...
                  <TrendingUp className={`w-8 h-8 ${stat.color} opacity-20`} strokeWidth={2} />
                </div>
              </div>
            </Card>
          ))}

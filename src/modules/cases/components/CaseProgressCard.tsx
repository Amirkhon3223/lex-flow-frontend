...
export function CaseProgressCard({
  ...
  return (
    <Card>
      ...
      <Progress
        value={progress}
        className="h-1.5 sm:h-2 mb-3 sm:mb-4 bg-gray-200 [&>div]:bg-blue-500"
      />
      <div className="grid grid-cols-4 gap-3 sm:gap-4 text-center">
        {progressStats.map((stat) => (
          <div key={stat.labelKey} className="flex flex-col items-center">
            <Badge
              className={`${stat.color} border-0 text-base sm:text-lg px-2.5 sm:px-3 py-1 mb-1.5 sm:mb-2`}
            >
              {stat.count}
            </Badge>
